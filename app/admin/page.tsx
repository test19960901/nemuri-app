...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    onAuthStateChanged(auth, async (u) => {
      ...
      if (u) {
        ...
        setCards(cardsSnap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a: any, b: any) => a.cardNumber - b.cardNumber));
      }
    });
  }, []);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    catch (err: any) { alert("ログイン失敗: " + err.message); }
  };
  ...
  const addCard = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const imageUrl = await uploadImage(file, "cards");
      await addDoc(collection(db, "cards"), { cardNumber, title, keyword, imageUrl });
      alert("カードを追加しました！");
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <form onSubmit={handleLogin} className="bg-white p-6 rounded-2xl shadow-md w-full max-w-sm space-y-4">
          ...
          <input type="email" placeholder="Email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full p-2 border rounded-xl bg-slate-50" />
          <input type="password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} className="w-full p-2 border rounded-xl bg-slate-50" />
          <button type="submit" className="w-full py-2 bg-pink-500 text-white rounded-xl font-bold hover:bg-pink-600 transition">ログイン</button>
        </form>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-100 p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm">
          ...
          <button onClick={() => signOut(auth)} className="text-sm text-red-500 font-bold border border-red-500 px-4 py-1 rounded-full">ログアウト</button>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm space-y-4">
          ...
          <button onClick={saveConfig} className="bg-blue-500 text-white px-4 py-2 rounded-lg font-bold text-sm">基本設定を保存</button>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm space-y-4">
          ...
          <button onClick={saveMission} className="bg-blue-500 text-white px-4 py-2 rounded-lg font-bold text-sm">MISSIONを保存</button>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm space-y-4">
          ...
          <form onSubmit={addCard} className="bg-slate-50 p-4 rounded-xl space-y-3 border">
            ...
            <button type="submit" className="bg-pink-500 text-white px-4 py-2 rounded-lg font-bold text-sm">アップロードして追加</button>
          </form>
          ...
          <div className="grid grid-cols-4 gap-3 mt-4">
            {cards.map(c => (
              <div key={c.id} className="border rounded-lg p-2 flex flex-col items-center">
                ...
                <button onClick={() => deleteCard(c.id)} className="mt-2 text-red-500 text-xs font-bold border border-red-200 px-2 rounded hover:bg-red-50">削除</button>
              </div>
            ))}
          </div>
        </div>
